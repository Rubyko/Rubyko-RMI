...
import rubyko.java.rmi.server.RMIFailureHandler;
...
import rubyko.java.rmi.server.UID;
...
import rubyko.sun.rmi.transport.DGCAckHandler;
...
import rubyko.sun.rmi.transport.TransportConstants;
import rubyko.sun.rmi.transport.proxy.HttpReceiveSocket;
import rubyko.sun.security.action.GetIntegerAction;
import rubyko.sun.security.action.GetLongAction;
...
public class TCPTransport extends Transport {
	...
	private static final ExecutorService connectionThreadPool = new ThreadPoolExecutor(0, 2, 5000,
			TimeUnit.MILLISECONDS, new SynchronousQueue<Runnable>(), new ThreadFactory() {
				public Thread newThread(Runnable runnable) {
					return AccessController
							.doPrivileged(new NewThreadAction(runnable, "TCP Connection(idle)", true, true));
				}
			});
	...
	/**
	 * number of milliseconds in accepted-connection timeout. Warning: this
	 * should be greater than 15 seconds (the client-side timeout), and defaults
	 * to 2 hours. The maximum representable value is slightly more than 24 days
	 * and 20 hours.
	 */
	private static final int connectionReadTimeout = 2 * 3600 * 1000; // default
																		// 2
																		// hours
	...
	/**
	 * Returns a <I>Channel</I> that generates connections to the endpoint
	 * <I>ep</I>. A Channel is an object that creates and manages connections of
	 * a particular type to some particular address space.
	 * 
	 * @param ep
	 *            the endpoint to which connections will be generated.
	 * @return the channel or null if the transport cannot generate connections
	 *         to this endpoint
	 */
	...
	/**
	 * Removes the <I>Channel</I> that generates connections to the endpoint
	 * <I>ep</I>.
	 */
	...
	public void exportObject(Target target) throws RemoteException {
		/*
		 * Ensure that a server socket is listening, and count this export while
		 * synchronized to prevent the server socket from being closed due to
		 * concurrent unexports.
		 */
		synchronized (this) {
			listen();
			exportCount++;
		...
		/*
		 * Try to add the Target to the exported object table; keep counting
		 * this export (to keep server socket open) only if that succeeds.
		 */
		boolean ok = false;
		try {
	...
	/**
	 * Decrements the count of exported objects, closing the current server
	 * socket if the count reaches zero.
	 **/
	...
	private void listen() throws RemoteException {
		...
		if (server == null) {
			...
			try {
				 ...
				 */
				Thread t = new Thread(new AcceptLoop(server), "TCP Accept-" + port);
				t.start();
	...
	private class AcceptLoop implements Runnable {
		...
		public void run() {
			try {
				...
				try {
					/*
					 * Only one accept loop is started per server socket, so
					 * after no more connections will be accepted, ensure that
					 * the server socket is no longer listening.
					 */
					serverSocket.close();
				} catch (IOException e) {
				}
		...
		/**
		 * Accepts connections from the server socket and executes handlers for
		...
		private void executeAcceptLoop() {
			...
			while (true) {
				...
				try {
					...
					/*
					 * Execute connection handler in the thread pool, which uses
					 * non-system threads.
					 */
					try {
					...
					try {
						/*
						 * If the server socket has been closed, such as because
						 * there are no more exported objects, then we expect
						 * accept to throw an exception, so just terminate
						 * normally.
						 */
					...
					/*
					 * In case we're running out of file descriptors, release
					 * resources held in caches.
					 */
		...
		/**
		 * Returns true if the accept loop should continue after the specified
		 * exception has been caught, or false if the accept loop should
		 * terminate (closing the server socket). If there is an
		 * RMIFailureHandler, this method returns the result of passing the
		 * specified exception to it; otherwise, this method always returns
		 * true, after sleeping to throttle the accept loop if necessary.
		 **/
		...
		/**
		 * Throttles the accept loop after an exception has been caught: if a
		 * burst of 10 exceptions in 5 seconds occurs, then wait for 10 seconds
		 * to curb busy CPU usage.
		 **/
	 ...
	 * appropriately. If an exception occurs during message handling, the socket
	 * is closed.
	 */
	...
	private class ConnectionHandler implements Runnable {
		...
		/**
		 * int value of "POST" in ASCII (Java's specified data formats make this
		 * once-reviled tactic again socially acceptable)
		 */
		...
		/**
		 * Verify that the given AccessControlContext has permission to accept
		 * this connection.
		 */
