 ...
 * questions.
 */
package rubyko.sun.rmi.transport.tcp;
...
import rubyko.java.rmi.server.RMISocketFactory;
...
import rubyko.sun.rmi.transport.Channel;
import rubyko.sun.rmi.transport.Endpoint;
import rubyko.sun.rmi.transport.Target;
import rubyko.sun.rmi.transport.Transport;
...
public class TCPEndpoint implements Endpoint {
	...
	/**
	 * Find host name of local machine. Property "java.rmi.server.hostname" is
	 * used if set, so server administrator can compensate for the possible
	...
	static {
		...
		if (localHost == null) {
			try {
				...
				/*
				 * if the user wishes to use a fully qualified domain name then
				 * attempt to find one.
				 */
				if (getBoolean("java.rmi.server.useLocalHostName")) {
					...
					/*
					 * default to using ip addresses, names will work across
					 * seperate domains.
					 */
	...
	/**
	 * maps an endpoint key containing custom socket factories to their own
	 * unique endpoint
	 */
	...
	/**
	 * Create an endpoint for a specified host and port. This should not be used
	 * by external classes to create endpoints for servers in this VM; use
	 * getLocalEndpoint instead.
	 */
	 ...
	 * This should not be used by external classes to create endpoints for
	 * servers in this VM; use getLocalEndpoint instead.
	 */
	...
	/**
	 * Get an endpoint for the local address space on specified port. If port
	 * number is 0, it returns shared default endpoint object whose host name
	 * and port may or may not have been determined.
	 */
	...
	public static TCPEndpoint getLocalEndpoint(int port, RMIClientSocketFactory csf, RMIServerSocketFactory ssf) {
		 ...
		 * endpoints for this client/server socket factory pair (perhaps null)
			...
			if (epList == null) {
				...
				synchronized (epList) {
					...
					if (localHost != null && !localHost.equals(lastHost)) {
						/*
						 * Hostname has been updated; add updated endpoint to
						 * list.
						 */
						if (lastPort != 0) {
							/*
							 * Remove outdated endpoints only if the port has
							 * already been set on those endpoints.
							 */
							epList.clear();
						}
	...
	/**
	 * Resamples the local hostname and returns the possibly-updated local
	 * hostname.
	 */
	private static String resampleLocalHost() {
		...
		synchronized (localEndpoints) {
			...
			if (hostnameProperty != null) {
				if (!localHostKnown) {
					/*
					 * If the local hostname is unknown, update ALL existing
					 * endpoints with the new hostname.
					 */
					setLocalHost(hostnameProperty);
				} else if (!hostnameProperty.equals(localHost)) {
					/*
					 * Only update the localHost field for reference in future
					 * endpoint creation.
					 */
	...
	static void setLocalHost(String host) {
		...
		synchronized (localEndpoints) {
			/*
			 * If host is not known, change the host field of ALL the local
			 * endpoints.
			 */
			if (!localHostKnown) {
				localHost = host;
	...
	/**
	 * Set the port of the (shared) default endpoint object. When first created,
	 * it contains port 0 because the transport hasn't tried to listen to get
	 * assigned a port, or if listening failed, a port hasn't been assigned from
	 * the server.
	 */
	static void setDefaultPort(int port, RMIClientSocketFactory csf, RMIServerSocketFactory ssf) {
		...
		synchronized (localEndpoints) {
			...
			synchronized (epList) {
				...
				if (size > 1) {
					/*
					 * Remove all but the last element of the list (which
					 * contains the most recent hostname).
					 */
					epList.clear();
					epList.add(lastEp);
				}
			...
			/*
			 * Allow future exports to use the actual bound port explicitly (see
			 * 6269166).
			 */
	...
	/**
	 * Returns transport for making connections to remote endpoints; (here, the
	 * default transport at port 0 is used).
	 */
	...
	/**
	 * Returns the current list of known transports. The returned list is an
	 * unshared collection of Transports, including all transports which may
	 * have channels to remote endpoints.
	 */
	private static Collection<TCPTransport> allKnownTransports() {
		...
		synchronized (localEndpoints) {
			...
			for (LinkedList<TCPEndpoint> epList : localEndpoints.values()) {
				/*
				 * Each local endpoint has its transport added to s. Note: the
				 * transport is the same for all endpoints in the list, so it is
				 * okay to pick any one of them.
				 */
	...
	/**
	 * Release idle outbound connections to reduce demand on I/O resources. All
	 * transports are asked to release excess connections.
	 */
	...
	/**
	 * Returns the port for this endpoint. If this endpoint was created as a
	 * server endpoint (using getLocalEndpoint) for a default/anonymous port and
	 * its inbound transport has started listening, this method returns (instead
	 * of zero) the actual bound port suitable for passing to clients.
	 **/
	public int getPort() {
		return port;
	}
	...
	/**
	 * Returns the port that this endpoint's inbound transport listens on, if
	 * this endpoint was created as a server endpoint (using getLocalEndpoint).
	 * If this endpoint was created for the default/anonymous port, then this
	 * method returns zero even if the transport has started listening.
	 **/
	...
	/**
	 * Returns the transport for incoming connections to this endpoint, if this
	 * endpoint was created as a server endpoint (using getLocalEndpoint).
	 **/
	...
	public boolean equals(Object obj) {
		if ((obj != null) && (obj instanceof TCPEndpoint)) {
			...
			if (port != ep.port || !host.equals(ep.host))
				return false;
			if (((csf == null) ^ (ep.csf == null)) || ((ssf == null) ^ (ep.ssf == null)))
				return false;
			/*
			 * Fix for 4254510: perform socket factory *class* equality check
			 * before socket factory equality check to avoid passing a
			 * potentially naughty socket factory to this endpoint's
			 * {client,server} socket factory equals method.
			 */
			if ((csf != null) && !(csf.getClass() == ep.csf.getClass() && csf.equals(ep.csf)))
				return false;
			if ((ssf != null) && !(ssf.getClass() == ep.ssf.getClass() && ssf.equals(ep.ssf)))
				return false;
			return true;
		} else {
			return false;
	 ...
	 *                If id could not be read (due to stream failure)
	 */
	...
	/**
	 * Write endpoint to output stream in older format used by UnicastRef for
	...
	Socket newSocket() throws RemoteException {
		...
		try {
			...
			throw new rubyko.java.rmi.UnknownHostException("Unknown host: " + host, e);
		} catch (java.net.ConnectException e) {
			throw new rubyko.java.rmi.ConnectException("Connection refused to host: " + host, e);
		} catch (IOException e) {
	...
	/**
	 * The class FQDN encapsulates a routine that makes a best effort attempt to
	 * retrieve the fully qualified domain name of the local host.
	 *
	 * @author Laird Dornin
	 */
	private static class FQDN implements Runnable {
		...
		/**
		 * Do our best to obtain a fully qualified hostname for the local host.
		 * Perform the following steps to get a localhostname:
		 *
		 * 1. InetAddress.getLocalHost().getHostName() - if contains '.' use as
		 * FQDN 2. if no '.' query name service for FQDN in a thread Note: We
		 * query the name service for an FQDN by creating an InetAddress via a
		 * stringified copy of the local ip address; this creates an InetAddress
		 * with a null hostname. Asking for the hostname of this InetAddress
		 * causes a name service lookup.
		 *
		 * 3. if name service takes too long to return, use ip address 4. if
		 * name service returns but response contains no '.' default to
		 * ipaddress.
		 */
		...
		/**
		 * Method that that will start a thread to wait to retrieve a fully
		 * qualified domain name from a name service. The spawned thread may
		 * never return but we have marked it as a daemon so the vm will
