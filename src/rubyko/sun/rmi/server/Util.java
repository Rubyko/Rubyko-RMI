...
import rubyko.java.rmi.StubNotFoundException;
import rubyko.java.rmi.server.RemoteObjectInvocationHandler;
...
/**
 * A utility class with static methods for creating stubs/proxies and skeletons
 * for remote objects.
 */
...
public final class Util {
	...
	private static final boolean ignoreStubClasses = true;
	 ...
	 *
	 * If both of the following criteria is satisfied, a dynamic proxy for the
	 * specified implClass is returned (otherwise a RemoteStub instance for the
	 * specified implClass is returned):
	 *
	 * a) either the property java.rmi.server.ignoreStubClasses is true or a
	 * pregenerated stub class does not exist for the impl class, and b)
	 * forceStubUse is false.
	 *
	 * If the above criteria are satisfied, this method constructs a dynamic
	 * proxy instance (that implements the remote interfaces of implClass)
	 * constructed with a RemoteObjectInvocationHandler instance constructed
	 * with the clientRef.
	 *
	 * Otherwise, this method loads the pregenerated stub class (which extends
	 * RemoteStub and implements the remote interfaces of implClass) and
	 * constructs an instance of the pregenerated stub class with the clientRef.
	 *
	 * @param implClass
	 ...
	 *             if problem locating/creating stub or creating the dynamic
	 *             proxy instance
	 **/
	public static Remote createProxy(Class<?> implClass, RemoteRef clientRef, boolean forceStubUse)
		...
		if (forceStubUse || !ignoreStubClasses) {
	 ...
	 * 
	 * @throws ClassNotFoundException if no class is found to have a remote
	 * interface
	 */
	...
	/**
	 * Returns an array containing the remote interfaces implemented by the
	 * given class.
	 *
	 * @param remoteClass
	 *            the class to obtain remote interfaces from
	...
	/**
	 * Fills the given array list with the remote interfaces implemented by the
	 * given class.
	 *
	 * @throws IllegalArgumentException
	 *             if the specified class implements any illegal remote
	 *             interfaces
	 * @throws NullPointerException
	...
	private static void getRemoteInterfaces(ArrayList<Class<?>> list, Class<?> cl) {
		...
		for (int i = 0; i < interfaces.length; i++) {
			...
			/*
			 * If it is a remote interface (if it extends from java.rmi.Remote)
			 * and is not already in the list, then add the interface to the
			 * list.
			 */
	 ...
	 * type that is RemoteException or one of its superclasses. If not, then
	...
	/**
	 * Creates a RemoteStub instance for the specified class, constructed with
	 * the specified RemoteRef. The supplied class must be the most derived
	 * class in the remote object's superclass chain that implements a remote
	 * interface. The stub class name is the name of the specified remoteClass
	 * with the suffix "_Stub". The loading of the stub class is initiated from
	 * class loader of the specified class (which may be the bootstrap class
	 * loader).
	 **/
	private static RemoteStub createStub(Class<?> remoteClass, RemoteRef ref) throws StubNotFoundException {
		...
		/*
		 * Make sure to use the local stub loader for the stub classes. When
		 * loaded by the local loader the load path can be propagated to remote
		 * clients, by the MarshalOutputStream/InStream pickle methods
		 */
		try {
	 ...
	 * string of the method name and descriptor.
	 */
	...
	/**
	 * Return a string consisting of the given method's name followed by its
	 * "method descriptor", as appropriate for use in the computation of the
	 * "method hash".
	 *
	 * See section 4.3.3 of The Java Virtual Machine Specification for the
	 * definition of a "method descriptor".
	 */
	...
	/**
	 * Get the descriptor of a particular type, as appropriate for either a
	 * parameter or return type in a method descriptor.
	 */
	 ...
	 * Nested types are treated no differently from top-level types, so for a
	 * nested type, the returned name will still be qualified with the simple
	 * name of its enclosing top-level type (and perhaps other enclosing types),
